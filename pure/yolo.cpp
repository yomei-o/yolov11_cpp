...
// Ultralytics data.yaml. No Python at run time. yolo11 head is anchor-free DFL, so loss/
// TAL/decode are the yolov8 ones; forward is net11_unfused (C3k2 + C2PSA attention).
...
//   export --weights best.pt                      (delegates to onnx_export11 — see note)
// build: cc11.sh -std:c++20 -O2 -EHsc -Ipure/third_party pure/yolo.cpp
// NOTE: yolo11's C2PSA attention is memory-heavy; use --imgsz <= 96 on limited RAM.
...
static Arch11 load_arch11() {
  Arch11 ARC; std::ifstream f(DN + "arch11.txt"); f >> ARC.psa_n;
  int64_t n,c,i; while (f >> n >> c >> i) ARC.c3.push_back({n,i,(bool)c}); return ARC;
}
// load engine from a .pt (falls back to .bin export)
static ProviderU load_model(const std::string& weights) {
...
// forward one (1,3,S,S) image -> packed pd (A,4*RM) / ps (A,NC0)
static void forward_img(const Tensor& x, ProviderU& prov, Arch11& ARC, int64_t A, Tensor& pd, Tensor& ps) {
  prov.i = 0; auto h = yolo11n_forward_u(x, prov, false, ARC);
...
static double run_val(Dataset& va, ProviderU& prov, Arch11& ARC, int64_t S) {
  ...
  for (auto& s : va.items) {
    ...
    Tensor pd, ps; forward_img(xi, prov, ARC, A, pd, ps);
...
static int cmd_train(const Args& a) {
  ...
  std::string weights = a.get("weights", "yolo11n.pt");
  ...
  for (int ep = 0; ep < EPOCHS; ++ep) {
    ...
    for (size_t off = 0; off < order.size(); off += BATCH) {
      ...
      prov.i=0; auto h = yolo11n_forward_u(bt.x, prov, true, ARC);
      std::vector<Tensor> bx={h[0].first,h[1].first,h[2].first}, cs={h[0].second,h[1].second,h[2].second};
...
static int cmd_val(const Args& a) {
  DataYaml dy = parse_yaml(a.get("data"));
  int64_t S = a.geti("imgsz", 640);
  auto prov = load_model(a.get("weights", "yolo11n.pt")); Arch11 ARC = load_arch11();
  Dataset va = read_yolo_dataset(join(dy.path, dy.val), S);
  run_val(va, prov, ARC, S);
  return 0;
}
...
static int cmd_detect(const Args& a) {
  ...
  std::vector<float> axv,ayv,sv; make_anchors(S, axv,ayv,sv); int64_t A=(int64_t)sv.size();
  Tensor pd, ps; forward_img(x, prov, ARC, A, pd, ps);
  std::vector<float> pred; decode_predictions(pd->data, ps->data, axv,ayv,sv, A, NC0, RM, pred);
...
int main(int argc, char** argv) {
  ...
  if (cmd == "export") { printf("export: use `onnx_export11 <model>` (onnxruntime-verified). "
