// yolo11 training CLI — a real training loop, pure C++ / no Python at run time:
...
// (via the pure-C++ .pt writer). yolo11's head is anchor-free DFL, so the loss/TAL/decode
// are the yolov8 ones, reused verbatim. Initial weights come from a .pt read in C++.
//   build: cc11.sh -std:c++20 -O2 -EHsc -Ipure/third_party pure/train_cli.cpp
...
// decode head predictions (grid units) to image-unit xyxy for TAL, + sigmoid scores
...
int main(int argc, char** argv) {
  setvbuf(stdout, nullptr, _IONBF, 0);   // stream per-epoch lines even when redirected/backgrounded
  ...
  std::string initpt = argc>5?argv[5]:"yolo11n.pt";
  ...
  // state_dict KEY per engine tensor (engine emit order != state_dict order -> pair by name)
  ...
  auto validate = [&]() -> double {
    ...
    for (auto& s : va.items) {
      ...
      prov.i=0; auto h = yolo11n_forward_u(xi, prov, false, ARC);
