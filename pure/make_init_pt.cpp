...
//   build: cl /std:c++20 /O2 /EHsc /Ipure\third_party pure\make_init_pt.cpp
...
//               pass pure/ref/arch/yolo11s/ (etc.) to generate weights for another SIZE.
#include "ptio.hpp"
#include <cstdio>
#include <fstream>
#include <random>
...
int main(int argc, char** argv) {
  ...
  std::string pre  = argc > 3 ? argv[3] : "yolo11n.pt";
  const std::string DN = argc > 4 ? argv[4] : "pure/ref/data_net/";

  // architecture: one manifest line per layer, "kind Co Ci k s pad g eps act"; names.txt
  // lists the state_dict key for each emitted tensor, in the same (engine) order.
  std::ifstream mf(DN + "manifest_unfused.txt");
  if (!mf) { printf("cannot open %smanifest_unfused.txt (run export_unfused11.py once)\n", DN.c_str()); return 1; }
  ...
  for (int i = 0; i < nlayers; ++i) {
    int kind; int64_t Co, Ci, k, s, pad, g; double eps; int act;
    mf >> kind >> Co >> Ci >> k >> s >> pad >> g >> eps >> act;
    double fan_in = (double)Ci * k * k;                    // Ci is already per-group in the manifest
